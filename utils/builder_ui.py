...
def render_formulation_builder():
    ...
    for i in range(4):
        ...
        options = [""] + st.session_state.available_apis
        default_value = options[0] if i >= len(options) or options[0] not in options else options[0]
        name = col1.selectbox(
            f"API #{i+1}",
            options,
            key=f"api_name_{i}_{product_type}_{shape}_{quantity}"
        )
        strength = col2.number_input(
            f"Strength (mg/unit)", min_value=0.0, step=0.1,
            key=f"api_strength_{i}_{product_type}_{shape}_{quantity}"
        )
