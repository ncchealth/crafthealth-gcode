...
if page == "Formulation Builder":
    st.title("💊 NCC G-code Generator (Full Version)")
    ...
    quantity = st.number_input("Quantity", min_value=1, value=300)
    ...
    if st.button("Generate Formula") and apis:
        ...
        final_df = pd.concat([api_df[["name", "percentage", "total_mg", "ingredient_type"]], excipient_df[["name", "percentage", "total_mg", "ingredient_type"]]]).reset_index(drop=True)

        st.success(f"✅ Formula generated! Unit size: ~{required_unit_weight:.1f} mg | Flavour: {flavour}")
        ...
        gcode = [
            "; Craft Health G-code",
            f"; Type: {product_type} | Shape: {shape} | Qty: {quantity} | Scaled Unit: {required_unit_weight:.1f} mg | Flavour: {flavour}",
            "G21", "G90", "G28", ""
        ]
