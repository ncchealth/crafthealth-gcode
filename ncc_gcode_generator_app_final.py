...
# Final working version of NCC G-code Generator
# Includes: Formulation Builder, API/flavour selector, 100% formula correction, persistent download buttons, admin panel
...
if page == "Formulation Builder":
    st.title("💊 NCC G-code Generator (Final Version)")

    product_type = st.selectbox("Product Type", list(st.session_state.base_templates.keys()))
    shape = st.selectbox("Shape", ["Round", "Caplet", "Oval"])
    ...
    for i in range(4):
        ...
        with col1:
            name = st.selectbox(f"API #{i+1}", [""] + st.session_state.available_apis, key=f"api_name_{i}")
        with col2:
            strength = st.number_input(f"Strength (mg/unit)", min_value=0.0, step=0.1, key=f"api_strength_{i}")
    ...
    if st.button("Generate Formula"):
        ...
        api_total_percent = api_df["percentage"].sum()
        excipient_fraction = (100 - api_total_percent) / 100

        excipients = st.session_state.base_templates[product_type]
        excipient_df = pd.DataFrame(excipients)
        excipient_df["percentage"] = excipient_df["percentage"] * excipient_fraction
        ...
        st.session_state.generated_product_type = product_type
        st.session_state.generated_qty = quantity
        st.session_state.generated_unit_weight = required_unit_weight
        st.session_state.generated_flavour = flavour
        st.success("✅ Formula generated and ready to download below.")

    if "generated_df" in st.session_state:
        final_df = st.session_state.generated_df
        ...
        quantity = st.session_state.generated_qty

        gcode = [
            "; Craft Health G-code",
            f"; Type: {st.session_state.generated_product_type} | Shape: {shape} | Qty: {quantity} | Unit: {st.session_state.generated_unit_weight:.1f} mg | Flavour: {st.session_state.generated_flavour}",
            "G21", "G90", "G28", ""
        ]
        ...
        class PDF(FPDF):
            def header(self):
                ...
                self.cell(0, 10, f"{st.session_state.generated_product_type} - Formulation Worksheet", ln=True, align="C")
                self.set_font("Arial", "", 10)
                self.cell(0, 10, f"Date: {datetime.today().strftime('%d %b %Y')} | Qty: {st.session_state.generated_qty} | Unit Size: {st.session_state.generated_unit_weight:.1f} mg", ln=True, align="C")
                self.ln(10)
